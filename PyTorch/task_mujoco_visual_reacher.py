...
from sac_rad_agent import SACRADPerformer, SACRADLearner
import utils as utils
from Common.env import ReacherWrapper
from comm import MODE
...
def parse_args():
    ...
    parser.add_argument('--episode_length_time', default=75, type=int)
...
def main():
    ...
    args.work_dir += f'/results/pt_run3/'
    ...
    for step in range(args.env_steps + args.init_steps):
        ...
        if stat is not None:
            for k, v in stat.items():
                L.log(k, v, step)
                if k == "train/update_time":
                    train_times.append(v)
...
# Run 1
# Number of training steps: 23999
# Average time per training step: 0.06839392459304627

# Run 2
# Number of training steps: 23999
# Average time per training step: 0.06842454332446818